...
class ProactorSocketTransportTestsBase:
...
class ProactorSocketTransportWithProtocolTests(test_utils.TestCase, ProactorSocketTransportTestsBase):
    ...
    def test_ctor(self):
        ...
        self.proactor.recv.assert_called_with(self.sock, self.buffer_size)

    def test_loop_reading(self):
        tr = self.socket_transport()
        tr._loop_reading()
        self.loop._proactor.recv.assert_called_with(self.sock, self.buffer_size)
    ...
    def test_pause_resume_reading(self):
        ...
        self.protocol.data_received.assert_called_with(msgs[4])
        tr.close()

        self.assertFalse(tr.is_reading())


class ProactorSocketTransportWithBufferedProtocolTests(test_utils.TestCase, ProactorSocketTransportTestsBase):

    def make_test_protocol(self):
        self.protocol = test_utils.make_test_buffered_protocol(
            asyncio.BufferedProtocol, self.buffer_size)

        # patch for some test in the base class to pass
        # BufferedProtocol calls recv_into instead of recv and all the tests
        # that assert if the function is called check if `recv` is called.
        self.loop._proactor.recv = self.loop._proactor.recv_into

    def setUp(self):
        super().setUp()
        self.setup_globals()

    def assert_buffer_received(self, data):
        self.assertTrue(self.protocol.buffer_updated.called)
        self.assertEqual(self.protocol._last_called_buffer[:len(data)],
                         bytearray(data))
    ...
    def test_loop_reading_data(self):
        ...
        self.assert_buffer_received(buf)
    ...
    def test_pause_resume_reading(self):
        ...
        for msg in msgs[:2]:
            self.loop._run_once()
            self.assert_buffer_received(msg)
        ...
        for _ in range(10):
            self.loop._run_once()
        self.assert_buffer_received(msgs[1])
        ...
        self.loop._run_once()
        self.loop.call_exception_handler.assert_not_called()
        self.assert_buffer_received(msgs[4])
        tr.close()
